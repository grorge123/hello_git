...
ALLEGRO_BITMAP* start_img_enemy2;
ALLEGRO_BITMAP* start_img_enbullet;
ALLEGRO_BITMAP* rage;
...
typedef struct {
    ...
    bool hidden;
    // The pointer to the objectÅfs image.
    int hp;
    ALLEGRO_BITMAP* img;
} MovableObject;
...
#define MAX_BULLET 4
#define MAX_HEART 5
#define MAX_ENEMY_BULLET (3*MAX_ENEMY)
MovableObject plane;
...
MovableObject heart[MAX_HEART];
MovableObject enbullets[MAX_ENEMY_BULLET];
// [HACKATHON 2-4]
...
double last_hit_timestamp=0;
double last_enshoot_timestamp[MAX_ENEMY]={0};
double last_enemy_spawn;
...
void allegro5_init(void) {
    ...
    al_set_window_title(game_display, "I2P(I)_2020 Final Project <109062323>");
...
void game_init(void) {
    ...
    start_img_heart = al_load_bitmap("minecraft-heart.png");
    if(!start_img_heart)
        game_abort("u dumbass");
    start_img_plane = al_load_bitmap("plane.png");
    start_img_plane2 = al_load_bitmap("plane-hit.png");
    if (!start_img_plane)
        game_abort("failed to load image: plane.png");

    start_img_enemy = al_load_bitmap("smallfighter0006.png");
    start_img_enemy2= al_load_bitmap("smallfighter0006-2.png");
    start_img_enbullet=al_load_bitmap("pixel-bullet.png");
    rage=al_load_bitmap("rage-face.png");
    if (!start_img_enbullet)
        game_abort("failed to load image: pixel-bullet.png");

    start_bgm = al_load_sample("mythica.ogg");
    if (!start_bgm)
        game_abort("failed to load audio: mythica.ogg");

    pew = al_load_sample("heat-vision.wav");
    oof = al_load_sample("classic_hurt.wav");
    end=al_load_sample("end-music.wav");
    if (!oof)
        game_abort("failed to load audio: classic-hurt.wav");
...
void game_start_event_loop(void) {
    ...
    while (!done) {
        ...
        if(active_scene==SCENE_GAMEOVER)
        {
            ...
            al_play_sample(end,1,0.0,1,ALLEGRO_PLAYMODE_ONCE,&end_id);
            al_rest(3.5);
            game_abort("game ended");
        }
...
void game_update(void) {
    if (active_scene == SCENE_START) {
        ...
        {
            if(enbullets[i].hidden)
                continue;
            enbullets[i].y+=enbullets[i].vy;
            if(enbullets[i].y>600)
            {
                enbullets[i].y=-enbullets[i].h;
                enbullets[i].hidden=true;
            }
        }
        for (i=0;i<MAX_ENEMY;i++) {
            if (enemies[i].hidden)
                continue;
            enemies[i].vx=2*cos((enemies[i].y+(enemies[i].h/2))/20);
        ...
        for(i=0;i<MAX_BULLET;i++)
        {
            ...
            {
                if(bullets[i].y-enemies[j].y<bullets[i].h/2+enemies[j].h/2&&enemies[j].y-bullets[i].y<bullets[i].h/2+enemies[j].h/2)
                {
                    if(bullets[i].x-enemies[j].x<bullets[i].w/2+enemies[j].w/2&&enemies[j].x-bullets[i].x<bullets[i].h/2+enemies[j].h/2)
                    {
                        enemies[j].hp--;
                        bullets[i].hidden=true;
                        bullets[i].y=800;
                        if(enemies[j].hp==0) enemies[j].hidden=true,last_enemy_spawn=now,enemies[j].y=-enemies[i].h;
                        else enemies[j].img=start_img_enemy2;
                    }
                }
            }
        }
        for(i=0;i<MAX_BULLET;i++)
        {
            ...
            {
                if(bullets[i].y-enbullets[j].y<bullets[i].h/2+enbullets[j].h/2&&enbullets[j].y-bullets[i].y<bullets[i].h/2+enbullets[j].h/2)
                {
                    if(bullets[i].x-enbullets[j].x<bullets[i].w/2+enbullets[j].w/2&&enbullets[j].x-bullets[i].x<bullets[i].h/2+enbullets[j].h/2)
                    {
                        enbullets[j].hidden=true;
                        bullets[i].hidden=true;
                        bullets[i].y=800;
                        enbullets[j].y=-enbullets[j].h;
                    }
                }
            }
        }
        for(i=0;i<MAX_ENEMY;i++)
        {
            if(plane.y-enemies[i].y<(enemies[i].h+plane.h)/2&&enemies[i].y-plane.y<(enemies[i].h+plane.h)/2&&!enemies[i].hidden)
            {
                if(plane.x-enemies[i].x<(enemies[i].w+plane.w)/2&&enemies[i].x-plane.x<(enemies[i].w+plane.w)/2)
                {
                     if(now-last_hit_timestamp>=1.5f)
                    {
                        last_hit_timestamp=now;
                        plane.img=start_img_plane2;
                        plane.hp--;
                        al_play_sample(oof,1,0.0,1,ALLEGRO_PLAYMODE_ONCE,&oof_id);
                        game_log("hp remain: %d",plane.hp);
                        heart[plane.hp].hidden=true;
                    }
                }
            }
        }
        for(i=0;i<MAX_ENEMY_BULLET;i++)
        {
            if(plane.y-enbullets[i].y<(enbullets[i].h+plane.h)/2&&enbullets[i].y-plane.y<(enbullets[i].h+plane.h)/2)
            {
                if(plane.x-enbullets[i].x<(enbullets[i].w+plane.w)/2&&enbullets[i].x-plane.x<(enbullets[i].w+plane.w)/2)
                {
                     if(now-last_hit_timestamp>=1.5f)
                    {
                        enbullets[i].hidden=true;
        ...
        if(now-last_hit_timestamp>=1.5f) plane.img=start_img_plane;
        if(plane.hp==0) active_scene=SCENE_GAMEOVER;

        for(i=0;i<MAX_ENEMY;i++)
        {
            if(now-last_enshoot_timestamp[i]>=2.875f&&!enemies[i].hidden)
            {
                for(j=0;j<MAX_ENEMY_BULLET;j++)
                {
                    if(enbullets[j].hidden)
                    {
                        last_enshoot_timestamp[i]=now;
                        enbullets[j].hidden=false;
                        enbullets[j].x=enemies[i].x;
                        enbullets[j].y=enemies[i].y+enemies[i].h/2;
                        break;
                    }
                }
            }
        ...
        if (key_state[ALLEGRO_KEY_SPACE] && now - last_shoot_timestamp >= MAX_COOLDOWN) {
            for (i = 0; i<MAX_BULLET;i++) {
                if (bullets[i].hidden) {
                    ...
                    al_play_sample(pew,1,0.0,1,ALLEGRO_PLAYMODE_ONCE,&pew_id);
                    break;
                }
            }
        }
        ...
        for(i=0;i<MAX_ENEMY;i++) {
            if(enemies[i].hidden&&now-last_enemy_spawn>=1.0f) {
                last_enemy_spawn=now;
                enemies[i].x = (enemies[i].w / 2 + 15) + (float)rand() / RAND_MAX * (SCREEN_W - enemies[i].w-30);
...
void game_draw(void) {
    if (active_scene == SCENE_MENU) {
        ...
        for(i=0;i<MAX_HEART;i++)
            draw_movable_object(heart[i]);
        for (i = 0; i < MAX_ENEMY; i++)
            draw_movable_object(enemies[i]);
        for(i=0;i<MAX_ENEMY_BULLET;i++)
            draw_movable_object(enbullets[i]);
    }
    // [HACKATHON 3-9]
    ...
    else if (active_scene==SCENE_GAMEOVER)
    {
        ...
        al_draw_bitmap(rage,100,60,0);
        al_draw_text(font_pirulen_32,al_map_rgb(255,255,255),268,36,0,"GAME OVER");
    }
    al_flip_display();
}
...
void game_destroy(void) {
    ...
    al_destroy_bitmap(start_img_enemy2);
    al_destroy_bitmap(start_img_enbullet);
    al_destroy_bitmap(rage);
...
void game_change_scene(int next_scene) {
    ...
    if (active_scene == SCENE_MENU) {
        ...
        for(i=0;i<MAX_HEART;i++)
        {
            heart[i].img=start_img_heart;
            heart[i].w=al_get_bitmap_width(start_img_heart);
            heart[i].h=al_get_bitmap_height(start_img_heart);
            heart[i].x=i*(heart[i].w+10)+heart[i].w/2;
            heart[i].y=heart[i].h/2;
            heart[i].vx=0;
            heart[i].vy=0;
            heart[i].hidden=false;
        }
        for (i = 0; i < MAX_ENEMY; i++) {
            enemies[i].img = start_img_enemy;
            ...
            enemies[i].x = (enemies[i].w / 2 + 15) + (float)rand() / RAND_MAX * (SCREEN_W - enemies[i].w-30);
            enemies[i].y = -enemies[i].h/2-100*i;
            enemies[i].vy=1;
            enemies[i].hp=2;
            enemies[i].hidden=false;
        }
        for(i=0;i<MAX_ENEMY_BULLET;i++)
        {
            enbullets[i].img=start_img_enbullet;
            enbullets[i].w=al_get_bitmap_width(start_img_enbullet);
            enbullets[i].h=al_get_bitmap_height(start_img_enbullet);
            enbullets[i].x=-enbullets[i].w;
            enbullets[i].y=-enbullets[i].h;
            enbullets[i].vy=2;
            enbullets[i].hidden=true;
        }
