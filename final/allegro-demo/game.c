...
//TODO create menu_scene

void game_create() {
    // Set random seed for better random outcome.
    srand(time(NULL));
    pthread_t shar_init, load_start, load_setting, load_menu, load_gameover, load_gamewin;
    allegro5_init();
...
void game_change_scene(Scene* next_scene) {
    ...
    if (active_scene.initialize && !active_scene.init){
        game_log("active scene init");
        next_scene->init = true;
